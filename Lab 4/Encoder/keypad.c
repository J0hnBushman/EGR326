...
*Brief: 
****************************************/
void Print_Keys (uint16_t *numptr, uint16_t *digitptr){
...
void gpio_init(void){
		...
		RCC->APB2ENR |= 1<<14; //enable the sysconfig
	
		//set the clk, DT, and SW as inputs
		GPIOC->MODER &= ~(0x03<<(2*CLK));
		GPIOC->MODER &= ~(0x03<<(2*DT));
		GPIOC->MODER &= ~(0x03<<(2*SW));
		
	//set the R,G,B LED pins
		GPIOC->MODER &= ~(0x03<<(2*R));
	  GPIOC->MODER &= ~(0x03<<(2*G));
	  GPIOC->MODER &= ~(0x03<<(2*B));
		GPIOC->MODER |= (0x01<<(2*R));
	  GPIOC->MODER |= (0x01<<(2*G));
	  GPIOC->MODER |= (0x01<<(2*B));
	
		//sysconfig for interupts on pin 1 and 5
		SYSCFG->EXTICR[0] &= ~0xF0;
	  SYSCFG->EXTICR[1] &= ~0xF0;
